...
> Applies to program version: v0.5.0 (M3U8D)
...
The program entry points and main execution chain are as follows:
    - main.py Responsible for starting `QApplication`, creating the main window, and parsing the `--url` / `--headers` / `--filename` parameters.
    - ui/main_window.py Responsible for assembling the browser tab, resource tab, and download-management tab into the main UI and connecting all signals.
    - core/playwright_driver.py Responsible for launching persistent Chrome, listening to page navigation, network requests, responses, and results returned by injected frontend scripts.
    - core/download_context.py Defines the unified ``EngineSelectContext`` that bundles url, page_url, page_title, source, resource_type, mime, headers, master_url, media_url, and metadata. Both the CatCatch path and the internal-browser path construct this context early and pass it through the entire sniff → engine-select → download pipeline, enabling context-aware engine selection.
    - core/engine_selector.py Responsible for choosing the best download engine for each resource. The decision chain is: user preference → context-aware detection (resource_type / mime) → HEAD MIME probe → extension fallback → live-platform list → yt-dlp fallback.
    - core/m3u8_sniffer.py Responsible for receiving discovered resources in a unified way, normalizing headers, applying site rules, deduplicating, and merging context.
    - ui/resource_panel.py Responsible for displaying the resource table, filtering, batch operations, and background M3U8 parsing plus variant expansion.
    - core/download_manager.py Responsible for the task queue, concurrency, retries, fallback, HLS preflight probing, state transitions, and download metrics.
    - engines/*.py Responsible for dispatching tasks to N_m3u8DL-RE, yt-dlp, Streamlink, and Aria2 respectively.
    - ui/download_queue.py / ui/log_panel.py / ui/history_panel.py Respectively responsible for task-queue display, key runtime-log display, and download history with re-download support.
    - core/catcatch_server.py + protocol_handler.pyw Responsible for integrating with the browser extension and system protocol handler, sending external requests into the program.
...
2.2 Startup with Command-Line Parameters
    The current entry point actually supports the following parameters:
        --url
            A video or page URL passed in externally. Validation: only `http` / `https` schemes are accepted, and the length must be at most 4096 characters. The value is filtered for private-network and cloud-metadata targets (addresses such as 127/10/172.16-31/192.168/169.254/::1/fc00::/fe80:: and cloud-metadata endpoints like 169.254.169.254 are rejected). When validation fails, the program exits with code 2 and does not start the UI.

        --headers
            A JSON string, typically in the following format: {"referer":"...","user-agent":"...","cookie":"..."} Validation: the headers go through the same sanitation path as the CatCatch HTTP service — header names may only contain `[A-Za-z0-9-]` and must be at most 64 characters; values must not contain `\r \n \0` and must be at most 4096 characters; only the allowlisted fields Referer / User-Agent / Origin / Cookie / Accept-Language are kept, and any other fields are silently dropped.

        --filename
            The default filename or title for the incoming resource. Validation: Windows reserved names (CON / PRN / AUX / NUL / COM1-9 / LPT1-9), ASCII control characters, and trailing `.` / whitespace are stripped automatically; the final absolute-path byte length is capped at 240; when the value ends up fully empty, it falls back to `media_<timestamp>`.
...
3.0 Component Manager and Auto-Update Notes
    ...
    Installer and missing components:
        When using the installer, required components (yt-dlp, N_m3u8DL-RE, and FFmpeg) are downloaded by default. Recommended components such as aria2c and Streamlink need to be selected in the installation wizard. If a component is later found missing, outdated, or has an abnormal path, open “Component Manager” to install / update it.
...
4.1 Browser Startup Mechanism
    Implementation:
        The browser is not a web control embedded inside the Qt window. Instead, Playwright launches a real persistent Chrome. Its core logic is in `core/playwright_driver.py`.
...
4.3 Back / Forward / Refresh / New Tab
    Current code status:
        - New Tab, Back, Forward, and Refresh are all implemented, calling Playwright's newPage / goBack / goForward / reload respectively.
    This means:
        - All four navigation buttons work normally.
...
4.5 How Browser Sniffing Is Implemented
    ...
    There are 4 main discovery paths inside `PlaywrightDriver`:
        1. Page navigation matches a video-page pattern For example, page URL rules for YouTube / Bilibili / TikTok / Instagram / Twitch, etc. After a match, the program adds the page URL to the list as a resource that yt-dlp can handle.
        2. `request` event Intercepts network requests sent by the browser. As long as the URL looks like a video stream, it is recorded.
        3. `response` event Even if the URL itself does not look like a video, it is still recorded when the response header `Content-Type` indicates HLS / MP4 / WebM, etc.
        4. Frontend injected script + `console` callback The page injects `sniffer_script`, which actively outputs frontend-discovered media addresses to the console during playback, and the Python side then receives and stores them.
...
4.7 Special Handling for Magnet Links
    Implementation:
        When a link starting with `magnet:?` is entered in the address bar, the browser is not instructed to navigate. Instead, the program directly constructs a resource entry and forcibly sets Aria2 as the suggested engine.
...
5.4 Download Selected / Remove Selected / Clear List / Clear Temp Files
    ...
    Clear Temp Files:
        A dedicated entry (button or menu item) that only cleans intermediate download artifacts (`.part` / `.tmp` / leftover segments) under `temp_dir`. It is no longer performed implicitly by the add-task path, so it will not delete segments of a task you are currently downloading. Run it manually when disk space is tight.
...
5.6 Routing Logic After Clicking Download
    ...
    Enqueue result feedback (four possible outcomes after clicking Download):
        Regardless of which branch (A / B / C) is taken, enqueue ultimately goes through the download manager's `add_resource`, and the UI surfaces one of the following four explicit outcomes so the user is never left wondering whether the click did anything:
...
6.3 Retry Count
    Corresponds to two levels:
        A. Retry count in the UI engines.n_m3u8dl_re.retry_count This value is written directly to N_m3u8DL-RE as the engine-internal retry parameter.
        B. Overall retry count in the download manager max_retry_attempts This value controls how many complete task-level rounds `DownloadManager` can attempt at most.
    Difference:
...
6.4 Concurrent Tasks
    ...
    Dynamic adjustment:
        When concurrency is lowered from N to M (M<N), the most recently started N-M workers receive a soft-exit signal and finish naturally once their current task completes. If any of them does not exit within 30 seconds, a `worker_exit_timeout` warning is recorded, but tasks that are already downloading are never force-killed. Raising concurrency spawns new workers immediately; the `active_workers` indicator in the UI always reflects the real live count.
...
6.7 Download Queue Context Menu and Bottom Buttons
    Currently supported:
        ...
        - Sort by Status
        - Batch Import
    Right-click menu also supports:
        - Copy Link
        - Completed tasks can play the file
        - Completed tasks offer FFmpeg post-processing (remux to MP4 / compress video / extract subtitles / merge audio)
...
7.2 Engine-Selection Priority
    Combined decision order (highest priority first):
        ...
        2. Context-aware detection: matching by resource_type and mime (no network round-trip needed).
        3. MIME probing via HEAD request (2 s timeout, protected by the private-network / cloud-metadata filter; on probe failure it falls back to extension inference and records `engine_select=fallback`).
        4. File-extension inference (the URL query string is stripped before matching).
        5. Live-platform list match (`LIVE_PLATFORMS`).
        6. yt-dlp as the final fallback.
    The decision tables (file extensions and live-platform list) are externalized in `resources/engine_rules.json` and can be edited directly without changing any code.
...
7.3 Task Enqueueing and Concurrency
    ...
    Flow:
        1. Idempotency precheck: the key `sha1(url|engine|out_dir|title)` is looked up first. If a task with the same key already exists, the new request is merged into it (returning `merged`) so repeated "re-download" clicks do not stack duplicate entries in the queue.
        2. Disk-space precheck: an estimated size is read from the manifest (falling back to 500 MB when it cannot be read) and compared with the free space on the disk that hosts the target directory. When space is insufficient, you are prompted with `needs_confirmation=insufficient_disk` and can choose to continue or cancel; choosing to bypass the precheck records `disk_precheck=bypassed`.
...
7.7 Pause, Resume, Cancel, and Delete Tasks
    Stop response time:
        After you click "Pause" or "Cancel", every engine (N_m3u8DL-RE / yt-dlp / Streamlink / Aria2) breaks out of its read loop and calls `terminate()` within 500 ms; if the process has not exited after 1.5 s, it is killed recursively. The typical end-to-end response time is at most 2 seconds. Cancelling during an FFmpeg merge of a large file cleans up intermediate `.part` / `.tmp` files while keeping the artifacts from the previous completed step (for example, if the segments have finished downloading but the merge has not completed, the segments are kept so you can resume later).
...
7.8 Download Metrics and Automatic Learning of Site Rules
    ...
    Automatic learning of site rules:
        ...
        When this feature is enabled, `referer` / `user-agent` / `origin` / `cookie` from successful tasks can be extracted as automatic rules, so headers can be auto-filled in future visits to the same site.
...
8.0 Context-Aware Automatic Engine Selection
    The current program has implemented context-aware engine selection. When a resource is detected by the sniffer, the detection source (CatCatch / internal browser / manual) and the resource type (hls / dash / direct_video / segment / page) are recorded.  This context flows through the unified ``EngineSelectContext`` (defined in ``core/download_context.py``) into the engine selector, which uses it to make decisions before resorting to HEAD MIME probes.

    The full priority chain is:
        1. User-specified engine (manual override)
        2. Context-aware detection by resource_type and mime
        3. HEAD MIME probe (2-second timeout)
        4. Extension-based fallback
        5. Live-platform URL pattern match
        6. yt-dlp catch-all fallback
    This means m3u8 resources are routed to N_m3u8DL-RE, mpd to N_m3u8DL-RE, direct video to Aria2, and page-based sites to yt-dlp — all without the extra network round-trip of a HEAD request.
...
8.1 N_m3u8DL-RE
    ...
    **Manual download**: To install manually, download `*win-x64*.zip` from the GitHub Release, extract `N_m3u8DL-RE.exe`, and place it in the `bin` directory:
    - Repository: https://github.com/nilaoda/N_m3u8DL-RE
    - Release page: https://github.com/nilaoda/N_m3u8DL-RE/releases
8.2 yt-dlp
    Suitable for:
    ...
    Characteristics of the current implementation:
        ...
        - Supports reading browser Cookies: it can use the Playwright Chromium user-data directory (`--cookies-from-browser chromium:<data-dir>`), falling back to the system Chrome (`--cookies-from-browser chrome`) when that directory is absent; Firefox (`--cookies-from-browser firefox`) is also supported.
        ...
        - Supports generic impersonate (browser impersonation): `--extractor-args generic:impersonate` makes yt-dlp send requests with a real browser fingerprint, used to bypass Cloudflare and other fingerprint-based protections; in this scenario Origin / Cookie are also re-attached via `--add-header`.
    ...
    How Cookies are actually used:
        ...
        - When the exact match misses, the program falls back along `.` boundaries in the hostname: `music.youtube.com` first tries `music.youtube.com_cookies.txt`, then `youtube.com_cookies.txt`, then `com_cookies.txt`, and so on until a file is found or the suffix list is exhausted.
        - When no manual Cookies file is available, browser Cookies fallback (Chromium / Chrome / Firefox) is attempted.
    Console encoding:
        On Windows, yt-dlp stdout is decoded through a three-tier fallback: utf-8 (`errors='replace'`) → mbcs (the system ANSI code page, typically CP936) → latin-1. When the decoder actually falls back to mbcs / latin-1, the log line is tagged with `decode=mbcs` / `decode=lossy` so characters are never dropped silently.
    ...
    **Manual download**: To install manually, download `yt-dlp.exe` from the GitHub Release and place it in the `bin` directory:
    - Repository: https://github.com/yt-dlp/yt-dlp
    - Release page: https://github.com/yt-dlp/yt-dlp/releases
    - Latest direct link: https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe
8.3 Streamlink
    Suitable for:
    ...
    Characteristics of the current implementation:
        ...
        - Cookies are split automatically on `;`: `a=1; b=2` becomes two separate `--http-cookie "a=1"` and `--http-cookie "b=2"` arguments, values are URL-escaped, and entries with an empty name or without `=` are dropped.
    **Manual download**: To install manually, download `streamlink-*-x86_64.zip` from the GitHub Release, extract `streamlink.exe`, and place it in the `bin` directory:
    - Windows builds repository: https://github.com/streamlink/windows-builds
    - Release page: https://github.com/streamlink/windows-builds/releases
    - Main repository (source): https://github.com/streamlink/streamlink
8.4 Aria2
    Suitable for:
    ...
    **Manual download**: To install manually, download `*win-64bit*.zip` from the GitHub Release, extract `aria2c.exe`, and place it in the `bin` directory:
    - Repository: https://github.com/aria2/aria2
    - Release page: https://github.com/aria2/aria2/releases
8.5 FFmpeg
    Its actual role in the current code:
        `FFmpegProcessor` is already loaded. The right-click menu of completed tasks exposes post-processing actions: remux to MP4, compress video, extract subtitles, and merge audio. Post-processing runs in a background thread and does not block the UI.
    ...
    Implemented methods include:
        ...
        - Compress video
    Description:
        Accessed via the right-click menu on completed tasks in the download queue, serving as a post-processing entry point.

    Cancel during merge:
        Large-file merges run through a cancellable read loop. When the operation is cancelled, intermediate `.part` / `.tmp` files are cleaned up while artifacts from the previous completed step are kept (for example, if the segments have finished downloading but the merge has not completed, the segments are preserved so you can resume later).

    **Manual download**: To install manually, download `ffmpeg.exe` and place it in the `bin` directory:
    - Official repository: https://github.com/FFmpeg/FFmpeg
    - Windows build (recommended, default source used by the program): https://www.gyan.dev/ffmpeg/builds/  (download `ffmpeg-release-essentials.zip`, extract `bin/ffmpeg.exe`)
    - Release page: https://github.com/FFmpeg/FFmpeg/releases

### 8.6 Deno

Deno is a modern JavaScript/TypeScript runtime. M3U8D uses it to execute some sniffer scripts (e.g., TypeScript injection scripts when needed). It is an optional dependency; the sniffer still works without it, but advanced sniffer scripts for some sites may not run.

- Executable location: `bin/deno.exe`
- Auto-update is not configured for this component; it must be downloaded and maintained manually
**Manual download**: To install manually, download `deno-x86_64-pc-windows-msvc.zip` from the GitHub Release, extract `deno.exe`, and place it in the `bin` directory:
- Repository: https://github.com/denoland/deno
- Release page: https://github.com/denoland/deno/releases
...
9.1 Runtime Logs
    ...
    Default level and debug switches:
        File-level logging defaults to INFO. To make background file logs quieter, set `M3U8D_LOG_LEVEL=WARNING` (WARNING/ERROR/CRITICAL only) or `M3U8D_LOG_LEVEL=ERROR` (ERROR/CRITICAL only) and restart the program. To capture DEBUG output for troubleshooting, set `M3U8D_LOG_DEBUG=1`; `M3U8D_LOG_LEVEL` takes precedence when both are present. Sensitive headers in command lines and URLs (Cookie / Set-Cookie / Authorization / Proxy-Authorization / X-Session-Token / User-Agent / Referer / Origin) as well as URL query parameters named `token` / `sign` / `signature` / `auth` are replaced with `<redacted>` before log lines are written to disk, so they never land on disk in plaintext. When you need plaintext for troubleshooting, set `SECURITY_DEBUG=1` to enable a separate `debug.sensitive.log` (disabled by default; be sure to disable it again once troubleshooting is done). Logs rotate automatically by date and a new file is created when the day changes at midnight; when the total size exceeds the limit, a throttled rotation kicks in (checked every 1000 entries or every 5 seconds), so enabling high-frequency DEBUG will not slow down downloads.
...
9.2 Download History
    ...
    Sensitive-header stripping before writing to history:
        Before `history.json` is written to disk, the `headers` field is filtered through a denylist so that short-lived tokens / credentials are not persisted to the user directory:
            - The following keys (case-insensitive) are dropped entirely and will NOT appear in `history.json`: `Cookie` / `Set-Cookie` / `Authorization` / `Proxy-Authorization` / `X-Session-Token` / `X-Auth-Token` / `Token` / `Api-Key` / `X-Api-Key`.
...
9.3 System Notifications
    ...
    Actual behavior in the current code:
        As of v0.5.0, system notification toasts are displayed via Qt's native QSystemTrayIcon, no longer relying on plyer. This eliminates the CMD window flash on Windows. Cross-thread notifications are safely marshalled to the main thread via Qt signals.
...
10.1 CatCatch HTTP Service
    ...
    Authentication and cross-site protection:
        - At startup a one-time session token is generated at random and written to `~/.m3u8d/session.token` (POSIX permission 0600 / owner-only DACL on Windows) for trusted clients to read.
        - Every request's `Origin` / `Referer` must be on the allowlist; the default allowlist contains the four loopback variants `http://127.0.0.1`, `http://localhost`, `https://127.0.0.1`, and `https://localhost`.
        - `POST /download` additionally requires the `X-Session-Token` header to match the current session token: when it is missing or wrong the service returns 401; when the `Origin` is not on the allowlist it returns 403 and does not echo back any `Access-Control-Allow-*` header.
    ...
    Headers forwarded to engines are sanitized:
        - Header names may only contain `[A-Za-z0-9-]` and must be at most 64 characters; values must not contain `\r \n \0` and must be at most 4096 characters.
        - Only the allowlisted fields Referer / User-Agent / Origin / Cookie / Accept-Language are forwarded; other fields are silently dropped.
        - When forwarded to an engine command line, headers are always passed through a parameterized argument array; string concatenation is never used.
    Request size limit:
        The `POST /download` request body is capped at **64 KiB**. When that limit is exceeded the service returns **413 Request Entity Too Large** and writes a `catcatch_body_too_large` entry in the log. Normal browser-extension payloads (URL + headers + filename, typically a few KiB) sit comfortably below this ceiling and are never affected by it.
    ...
    Endpoints:
        ...
        POST /download
            Accepts JSON or form. Supported fields: `url` / `headers` / `name` / `filename`
...
10.2 `m3u8dl://` Protocol Handler
    ...
    Actual execution flow:
        ...
        2. Read `~/.m3u8d/session.token` for the `X-Session-Token` header, attach `Origin: http://127.0.0.1`, and POST `/download` to each candidate port from 9527 to 9539 in turn. The first port that returns a 2xx response counts as a successful handshake and no new main instance is launched.
        3. Only when every candidate port fails to hand off (the main program really is not running, the token file is missing, or the token is no longer valid) does the handler start `main.py` with `--url` / `--headers` / `--filename`.
        4. After the new process comes up, `send_to_app` is polled for up to 12 seconds to deliver the link into the new instance's resource list.
    Log redaction:
        `logs/protocol_handler.log` never records the token in plaintext and only keeps redacted metadata such as `token_loaded=<bool>` / `token_len=<int>` / `status_code=<int>` / `auth_ok=<bool>`.

    Emergency rollback:
        If some environment cannot use the handshake for the moment (for example, the token file is quarantined by security software while the main program is still running), you can set the environment variable `M3U8D_HANDOFF_LEGACY=1` to temporarily revert to the legacy behavior (no token read, no Origin header, only a 2xx check). Every such call writes an extra legacy marker line so you can spot it in the log. It is disabled by default; please unset the variable once diagnosis is done.
...
11.2 `site_rules` and Automatically Learned Rules
    ...
    site_rules_auto.allow_cookie
        Meaning: whether Cookies may also be written into rules during automatic learning. Reminder:
...
11.3 `features`: Per-Item Explanation of Feature Switches
    sniffer_rules_enabled
        Meaning: whether `site_rules` should be applied at the sniffing stage to supplement headers. Recommendation: generally keep it `true`.

    sniffer_dedup_enabled
        Meaning: whether resource deduplication is enabled. Recommendation: generally keep it `true`; otherwise the list may explode in volume.

    sniffer_filter_noise
        Meaning: a reserved noise-filter switch for the network interceptor. Description: the current main stream-capture pipeline is Playwright-based, and the QWebEngine interceptor is not the primary path.
    ...
    allow_segment_probe_soft_fail
        Meaning: during HLS preflight probing, whether a single TS-segment probe failure (e.g. some CDNs return 403/404 for segment HEAD requests while the playlist itself is reachable) is treated as a soft failure (warn only, do not block the download). Description: defaults to true, corresponding to the `hls_probe_soft_fail` log marker.
    ...
    m3u8_nested_depth
        Meaning: the maximum depth for parsing nested master playlists. Description: although it may not be explicitly written in the default configuration file, the code supports this feature.

    network_verify_tls
        Meaning: whether to verify HTTPS / TLS certificates. Description: enabled by default; both m3u8 parsing and HLS preflight probing read this option to decide whether to verify TLS. Only disable it when certificates are abnormal and the target is confirmed to be trusted.

    failure_strategy_enabled
        Meaning: master switch for the download failure strategy.

    rate_limit_low_concurrency_enabled
        Meaning: whether to automatically reduce Aria2 connections / N_m3u8DL-RE threads and retry when CDN rate limiting (429) is detected.

    download_rate_limit_backoff_multiplier
        Meaning: multiplier for reducing connection counts during rate-limit backoff. Default 3.

    adaptive_low_concurrency_enabled
        Meaning: whether to adaptively lower concurrency for specific hosts based on historical rate-limit records.

    hls_probe_extended_soft_fail_enabled
        Meaning: whether to treat a 429 from the HLS key probe as a soft failure (warn only, do not block download).

    enhanced_header_capture_enabled
        Meaning: whether to capture and forward extra browser-context headers (Sec-Fetch-*, etc.) to improve anti-hotlink success rates.

    exact_request_header_replay_enabled
        Meaning: whether to exactly replay the original request headers captured during sniffing when downloading.

    forward_cookie_headers
        Meaning: whether to forward Cookie headers to engines. Enabled by default.

    forward_authorization_headers
        Meaning: whether to forward Authorization headers to engines. Disabled by default (fail-closed); only enable when confirmed necessary.

    temporary_cookie_forwarding_enabled
        Meaning: whether to temporarily reuse browser cookies for download engines. Disabled by default.

    resource_domain_cookie_lookup_enabled
        Meaning: whether to look up matching cookies from the browser's cookie store by resource domain.

    segment_suppression_enabled
        Meaning: whether to suppress segment resources in the resource list (TS segments exceeding the threshold for the same group are collapsed instead of listed individually).

    segment_suppression_threshold
        Meaning: segment suppression threshold — when segments in the same group exceed this count they are collapsed. Default 3.

    segment_advanced_view_enabled
        Meaning: whether to display suppressed segments in the resource list (advanced view). Disabled by default.

    ephemeral_m3u8_refresh_enabled
        Meaning: whether to automatically refresh a signed URL that is about to expire. Disabled by default.

    ephemeral_m3u8_refresh_timeout_ms
        Meaning: timeout in milliseconds for ephemeral m3u8 refresh. Default 15000.

    playlist_diagnostics_enabled
        Meaning: whether to automatically analyze m3u8 playlists for DRM system types and signed-URL expiry risk during sniffing.
...
11.4 `engines.n_m3u8dl_re`
    ...
    low_concurrency_thread_count
        Meaning: thread count used as a low-concurrency fallback after rate limiting is detected. Default 1.
...
11.5 `engines.ytdlp` / `streamlink` / `aria2` / `ffmpeg`
    ...
    engines.aria2.low_connection_count
        Connection count used as a low-concurrency fallback after rate limiting is detected. Default 1.
...
11.6 Other Configuration Items
    notification_enabled
        Meaning: whether notification events are logged. Description: currently mainly affects whether the notification function executes its logging-notification logic.

    auto_delete_temp
        Meaning: whether temporary download artifacts are automatically cleaned after segment-based engine tasks complete successfully. Description: when enabled, the main download flow invokes temp-cache cleanup after N_m3u8DL-RE / ffmpeg tasks complete;
                     cleanup skips artifacts owned by other running or paused tasks to avoid deleting concurrent downloads. When disabled, temporary files are cleaned only through the manual “Clear Temp Files” entry.

    proxy.enabled / proxy.http / proxy.https
        Meaning: whether a unified proxy is enabled for download engines and lightweight network probes. Description: when enabled, the main download flow forwards the proxy to N_m3u8DL-RE, yt-dlp, Aria2, and Streamlink command-line arguments;
                     m3u8 parsing, HLS preflight probing, and direct-link HEAD size estimation use the same proxy settings. `proxy.http` / `proxy.https` must be valid http, https, socks4, socks5, or socks5h proxy URLs.
    ...
    security.allow_private_networks
        Meaning: whether m3u8 parsing, resource sniffing, HLS preflight probing, and HEAD MIME probing are allowed to reach internal / reserved addresses (127/10/172.16-31/192.168/169.254/::1/fc00::/fe80::, etc.). Description: defaults to false (non-public addresses are rejected per SSRF protection); set to true only when you genuinely need to access a public domain that a corporate reverse proxy forwards into an internal network. When enabled, the log prints an `ssrf_private_allowed` warning indicating a reduced-privilege path. Regular users should keep the default.
...
11.7 Environment Variables
    ...
    M3U8D_LOG_DEBUG
        Meaning: when set to `1`, raises the file-level log level from the default INFO to DEBUG. Note: keep it disabled during normal use, and only enable it when you need to capture detailed runtime traces.

    M3U8D_LOG_LEVEL
        Meaning: explicitly sets the file-log level, for example `WARNING` or `ERROR`. Use `WARNING` when you want a smaller `m3u8sniffer.log`; use `ERROR` when you only care about errors. This setting takes precedence over `M3U8D_LOG_DEBUG`.

    SECURITY_DEBUG
        Meaning: when set to `1`, enables a separate `debug.sensitive.log` that records the raw content of command lines and sensitive headers for troubleshooting. Note: this file lives alongside the main logs but in a dedicated file. Once troubleshooting is done, disable the variable or remove the file so sensitive content does not linger on disk.

    M3U8D_HANDOFF_LEGACY
        Meaning: when set to `1`, the protocol handler falls back to the legacy behavior (no token read, no Origin header, only a 2xx check), acting as an emergency rollback channel when `m3u8dl://` delivery misbehaves. Keep it disabled during day-to-day use.

    M3U8D_SECURITY_DIAGNOSTIC
        Meaning: when set to `1` together with `security.allow_weak_manifest_verification=true` in the configuration, component updates are allowed to relax manifest verification in diagnostic mode. Note: only use this for offline diagnosis; in production both switches must remain disabled.
...
13.7 Private or LAN Addresses Cannot Be Downloaded
    To reduce the risk of using this program to reach internal networks or cloud-metadata endpoints (`127.0.0.1`, `10.x`, `172.16/12`, `192.168.x`, `169.254.x`, `fc00::/7`, and similar ranges), m3u8 parsing and fetching reject non-public addresses by default. The log will show markers such as `SSRFBlocked` or `ssrf_blocked`.

    If you genuinely need to download through a public-facing domain that a corporate reverse proxy forwards into an internal network, set `allow_private_networks` to `true` under the `security` node in `config.json` and restart the program. The log will print a prominent warning to indicate that this is a reduced-privilege path. Regular users should keep the default of `false`.
...
The following items are capabilities that “exist in code but are not necessarily used frequently as complete outward-facing features in the main UI”:
    ...
    - Configuration items such as `auto_delete_temp` / `proxy` already exist and are integrated into the main flow: `auto_delete_temp` automatically cleans a task's temp fragments after a successful segment-engine (N_m3u8DL-RE / ffmpeg) download; `proxy` is forwarded to the N_m3u8DL-RE, yt-dlp, Aria2, and Streamlink command-line arguments, and m3u8 parsing, HLS preflight probing, and direct-link HEAD estimation also use the same proxy configuration.
...
Newly added and fully integrated modules (v0.5.0, 2026-06):
    - core/download_context.py \u2014 the unified EngineSelectContext dataclass is already fully connected into both CatCatch and internal-browser detection chains. Context flows end-to-end from resource discovery through engine selection into DownloadTask creation.
    - Context-aware engine selection in core/engine_selector.py \u2014 _decide_from_context() is fully active and inserted in the priority chain before HEAD MIME probes.
    - The dual-signal design in core/playwright_driver.py (old resource_detected + new resource_context_detected) ensures backward compatibility while enabling the new context-rich path.
    - Auto-backfill in _start_download() ensures that even legacy call sites (format dialogs, variant dialogs) inherit context from the originating M3U8Resource.
