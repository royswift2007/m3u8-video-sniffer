...
class ResourcePanel(QWidget):
    ...
    def _init_ui(self):
        ...
        intro = QLabel("查看嗅探到的媒体链接，并按类型或清晰度筛选。")
        ...
        # 批量操作按钮
        self.batch_download_btn = QPushButton("下载所选")
        self.batch_download_btn.setFixedWidth(86)
        ...
        self.batch_remove_btn = QPushButton("移除所选")
        self.batch_remove_btn.setFixedWidth(86)
        ...
        self.quality_filter.addItems(["全部清晰度", "2160", "1440", "1080", "720", "480", "360", "240", "音频"])
        ...
        # 设置初始列宽
        self.resource_table.setColumnWidth(0, 300)  # 文件名
        self.resource_table.setColumnWidth(1, 60)   # 类型
        self.resource_table.setColumnWidth(2, 80)   # 清晰度
        self.resource_table.setColumnWidth(3, 130)  # 来源域名
        self.resource_table.setColumnWidth(4, 100)  # 引擎
        self.resource_table.setColumnWidth(5, 75)   # 时间
        self.resource_table.setColumnWidth(6, 60)   # 操作
    ...
    def add_resource(self, resource: M3U8Resource, engine_name: str):
        ...
        download_btn.clicked.connect(lambda checked, idx=row:
                                     self._on_download_btn_clicked(idx))
        ...
        def on_parsed(variants):
            if variants and row < self.resource_table.rowCount():
                ...
                if heights:
                    ...
                    quality_text = f"{len(variants)}个变体"
    ...
    def _apply_filters(self):
        ...
        type_filter = self.type_filter.currentText() if hasattr(self, "type_filter") else "全部"
        source_filter = self.source_filter.currentText() if hasattr(self, "source_filter") else "全部"
        quality_filter = self.quality_filter.currentText() if hasattr(self, "quality_filter") else "全部"

        def _normalize_filter_value(value: str) -> str:
            if value and value.startswith("全部"):
                return "全部"
            return value

        type_filter = _normalize_filter_value(type_filter)
        source_filter = _normalize_filter_value(source_filter)
        quality_filter = _normalize_filter_value(quality_filter)
        ...
        for row in range(self.resource_table.rowCount()):
            ...
            if visible and type_filter != "全部" and type_text != type_filter:
                visible = False

            if visible and source_filter != "全部" and source_text != source_filter:
                visible = False

            if visible and quality_filter != "全部":
                if quality_filter.lower() not in quality_text:
                    visible = False
